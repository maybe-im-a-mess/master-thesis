...
This chapter provides a review of fundamental topics and literature related to the main concepts used in this thesis, such as \textit{information extraction}, \textit{retrieval-augmented generation and citation}, and \textit{dialogue management}. It also presents an overview of the current state of research in evaluation of LLM-based systems in the medical domain.

\section{Large Language Models in Healthcare}

As Large Language Models (LLMs) have shown promising results in various fields, their application in healthcare has also been researched extensively. 
...
Ensuring the reliability and safety of LLMs involves their ability to produce accurate and trustworthy information while avoiding the generation of false or misleading content and ensuring that critical information is not omitted. In the medical domain, this can include extracting relevant medical facts from patient health records, extracting drug interactions from experimental studies, or extracting phenotypic traits from genomic reports, among others.
...
Since the advances made in LLMs' performance in the task of information extraction, this thesis will focus solely on the LLM-based approaches. Among these, four methods will specifically be explored: 
\begin{itemize}
    ...
    \item \textit{Unified Information Extraction}: This method is also referred to as \textit{Schema-Guided information extraction}, and it involves prompting an LLM to extract information based on a predefined schema, which can be used as a guide for the extraction process and help the model to perform consistently and accurately.
\end{itemize}

Starting with the most basic approach, the \textit{naive LLM prompting}, it is important to note that this method suggests that an LLM will be given a single instruction on how to perform the given task of information extraction and will be provided with the textual data from which the information is to be extracted. The LLM is then expected to produce the desired output based on its understanding of the task and the input data. Despite being a simple approach, this method has been shown to be effective and outperform any complex prompting strategies\cite{hanInformationExtractionSolved2023, liRevisitingLargeLanguage2023}. 

\textit{Chain-of-Thought prompting} is considered to be a more advanced method, as it involves a two-step (or multi-step) thinking process from an LLM. The approach was first introduced by Wei et al. \cite{weiChainofThoughtPromptingElicits2022} and has shown that forcing a reasoning process before producing the final answer can enhance the performance of LLMs in complex reasoning tasks \cite{maChainThoughtExplicit2023}.
...
Lastly, the \textit{Unified Information Extraction (UIE)} method implies the use of a predefined schema for the output. This method was introduced in order to deal with a diverse range of extraction targets, differently structured outputs, and schemas tailored to specific use cases \cite{luUnifiedStructureGeneration2022}. Schemas help to guide the extraction process and ensure the output is structured and consistent. Tackling the lack of adaptability of the original UIE, Liang et al. \cite{liangSchemaParameterizedTools2025} proposed Schema as Parameterized Tools, which explores the capability of LLMs to use their tool-calling ability to enhance the process of information extraction. 
...
Since the safety and reliability of LLMs in the medical domain are crucial, there is a need to address their knowledge cutoff and mitigate the hallucinations in their responses. \textit{Retrieval-augmented generation} (RAG) was an approach introduced by Lewis et al.\cite{lewisRetrievalAugmentedGenerationKnowledgeIntensive2021} to address these hallucinations.
...
Building on these automatic approaches, the \textit{LLM-as-a-Judge} concept was introduced into the evaluation process to explore the use of LLMs for automated evaluation and benchmarking. 
...
Before the emergence of LLMs, there was an explicit need to detect intents and fill slots, alongside other classical Natural Language Understanding tasks. LLMs effectively narrowed these tasks into a single model.
...
To mitigate such cases, researchers re-introduced dialogue management in addition to the LLMs' attention-based architecture. For example, Shukuri et al.\cite{shukuriMetacontrolDialogueSystems2023} introduced a meta-control layer into LLM-based dialogue systems in the form of specialized prompts to keep the conversation aligned with predefined tasks.
...
This category also includes n-gram based metrics like \textit{BLEU}\cite{papineniBLEUMethodAutomatic2002}, or \textit{ROUGE}\cite{linROUGEPackageAutomatic2004}. 
...
However, these metrics often miss the semantic aspect of the generated output and are unable to capture the quality of the text in terms of its meaning, paraphrasing possibilities, and redundancy. 
...
The similarity between two words or concepts is quantified by computing the distance between their vector representations. 
...
Though bi-encoders are faster and more scalable due to the production of embeddings that can be indexed and reproduced, cross-encoders are reported to perform more precisely for a variety of NLP tasks\cite{reimersSentenceBERTSentenceEmbeddings2019a}.
