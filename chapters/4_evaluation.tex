...
This approach includes embedding-based similarity scoring combined with the Hungarian algorithm for optimal bipartite matching.
...
The third coverage approach prompts an LLM (\texttt{gpt-4o-mini}) to act as a judge, determining if each ground-truth fact is sufficiently covered by the conversational context. 
...
First, factual claims are separated from conversational text. An algorithm filters out pure questions, short sentences ($< 20$ characters), and standard conversational fillers (e.g., \textit{``Ich verstehe''}). 
...
Lastly, to monitor the reliability of the LLM evaluator itself, a failure rate tracks the proportion of conversations where the LLM judge failed or returned corrupted output:
\[
