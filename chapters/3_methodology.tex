...
We decided against manual annotation of the documents, as this approach would not be feasible with a growing number of available documents.
...
The generated ground truth was manually inspected for quality and relevance, which confirmed that the model was able to effectively identify key topics and facts that are essential for patient communication, while filtering out less relevant information. 
...
The full prompt is provided in Appendix~\ref{app:naive_extraction_prompt}.
...
This selection was deliberately curated to include a variety of providers and licensing structures, providing a balanced mix of proprietary and open-weight models.
...
This is contrasted with three open-source alternatives originating from different AI research organizations: \textit{Qwen3-32B}, \textit{Ministral-14B}, and \textit{GPT-OSS-20B}.
...
The agent is explicitly instructed to cite these sources whenever detailing specific clinical facts, permitting uncited responses only for general conversational phrases where the medical context is not strictly required. 
...
Specifically, reassuring linguistic cues (e.g., \textit{``kein Grund zur Sorge''}) incrementally decrease anxiety and build trust, while expressions of uncertainty or excessively long, complex answers increase anxiety and reduce clarity.
