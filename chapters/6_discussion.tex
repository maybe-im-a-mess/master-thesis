...
The evaluation results from Chapter 5.1 reveal a counter-intuitive finding regarding Information Extraction (IE) strategies: the highly capable GPT-5 Mini model achieves its peak performance using the least structured approach (Naive prompting), whereas the smaller Ministral-14B model benefits significantly from the heavily structured Universal Information Extraction (UIE) framework.

For GPT-5 Mini, structured methods such as Chain-of-Thought (CoT) and UIE appear to introduce unnecessary constraints. 
As one of the advanced frontier models, GPT-5 Mini possesses deep zero-shot comprehension and a vast repository of internalized medical knowledge. 
Forcing a defined schema or intermediate reasoning steps supposedly restricts its robust natural language understanding, causing the model to unintentionally filter out valid but nuanced medical entities. 
...
While standard natural language processing tasks often penalize such over-extraction, this recall bias can be a crucial feature in medical contexts. 
Under-extraction means missing a critical symptom, contraindication, or risk factor.
This carries severe clinical risks and can lead to misdiagnosis. 
...
Consequently, while GPT-5 Mini is blocked by these boundaries, Ministral-14B requires them to compensate for its more limited native extraction capabilities.

\subsection{Topic Coverage Gap}

The reported in Chapter 5.2 topic coverage hit rate of 24\textendash32\% might initially appear low, but it should not be interpreted as a system failure. 
...
This finding is a natural extension of the Retrieval-Augmented Generation (RAG) principles introduced in Chapter 2. 
...
Specifically, the performance gains from supervision scale with dataset size, showing little improvements on the smaller Dataset A but substantial gains on the more comprehensive Dataset B. 
...
Furthermore, the variation across synthetic patient personas is helpful. 
The ``Trauma History'' persona benefited the most from the supervised dialogue management.
This case exhibits highly evasive or topic-switching behavior in simulations, which frequently derailed the naive agent. 
...
This stability is due to the fact that the supervised architecture only adds structural prompts regarding dialogue state without altering the agent's fundamental grounding mechanism. 
The core RAG retrieval and citation instruction sets remain intact, proving that it is possible to impose strict conversational guardrails on an LLM without sacrificing its epistemological grounding and source adherence.
...
First of all, implementing a supervised dialogue management architecture is crucial for safety-critical applications. 
...
Additionally, the marked variation in compliance across different patient personas, the performance with ``Language Barrier'' and ``Trauma History'' profiles in particular, implies that real-world deployments need persona-adaptive dialogue management.
...
Finally, the observed citation coverage of approximately 30\% presents a visible deployment risk. 
...
Real human patient behavior may differ substantially in terms of verbosity, topic switching, emotional tone, and baseline medical literacy, potentially introducing conversational difficulties that the synthetic environment failed to capture.
...
Third, the ground truth used for evaluating Information Extraction was itself constructed dynamically using an LLM.
...
Fourth, the system was evaluated on a single domain and language, focusing strictly on German-language obstetric and anaesthetic documents. 
...
Based on the findings and limitations of this study, several ways for future research may be proposed:
\begin{itemize}
